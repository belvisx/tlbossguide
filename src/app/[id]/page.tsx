...
// 🚀 Adicionando generateStaticParams para gerar rotas dinâmicas corretamente
export function generateStaticParams() {
  return bosses.map((boss) => ({
    id: boss.name.toLowerCase().replace(/\s+/g, "-"), // 🔥 Garantindo que os IDs estejam formatados corretamente
  }));
}

// ✅ Transformando `BossPage` em um Server Component sem erro de `params`
export default function BossPage({ params }: Props) {
  const boss = bosses.find((b) => 
    b.name.toLowerCase().replace(/\s+/g, "-") === params.id
  );
